...
class ParameterRanges:
    ...
    INVERT_INPUT_OPTIONS = [False, False]
    ...
    # Hold final frame fraction range
    HOLD_FINAL_FRAME_FRACTION_RANGE = (0.8, 1.0)

    # Target duration range
    TARGET_N_SECONDS_RANGE = (16.0, 16.0)
...
def sample_random_config() -> FillConfig:
    ...
    return FillConfig(
        ...
        hold_final_frame_fraction=random.uniform(*ParameterRanges.HOLD_FINAL_FRAME_FRACTION_RANGE),
    )
...
def generate_param_filename(config: FillConfig, base_name: str, output_dir: Path = None) -> str:
    ...
    param_parts = [
        ...
        f"hf{config.hold_final_frame_fraction:.2f}"
    ]
...
def generate_variations(input_dir: Path, n_variations: int = 100, output_dir: Path = None):
    ...
    for i in range(n_variations):
        try:
            ...
            target_n_seconds = random.uniform(*ParameterRanges.TARGET_N_SECONDS_RANGE)
            ...
            print(f"  Target duration: {target_n_seconds:.1f}s")
            ...
            print(f"  Hold final frame fraction: {config.hold_final_frame_fraction:.2f}")

            # Create the animation
            create_animation(image_path, target_n_seconds, output_path, config)
...
cd /Users/xandersteenbrugge/Desktop/SD/church_sam_trace_videomapping
python many_fill_polygon.py sam_maps_fey 100 fill_animations/fey_fills_01
